import{statuses as e,normalizeEntry as t,validateEntry as n,filterEntries as r,searchEntries as o,sortEntries as a,keys as _}from"./domain.js"
...
;let u,m=[],p="All projects",h=null,f=!1,g=null;const b=s("#project-form"),y=s("#editor");function v(e,t,n){const r=document.createElement(e)
;return t&&(r.className=t),void 0!==n&&(r.textContent=n),r}function C(e){s("#live-region").textContent=e,s("#toast").textContent=e,
s("#toast").classList.add("show"),clearTimeout(u),u=setTimeout(()=>s("#toast").classList.remove("show"),2800)}function w(){g&&(g.textContent="Delete",
...
S()},t.append(e)}const n=s("#grid");n.replaceChildren();const q=s("#search").value.trim(),l=a(o(r(m,p),q),s("#sort").value);if(!l.length){const e=v("div","empty-state")
...
;const t=v("button","secondary",m.length?q?"Clear search":"Show all projects":"Add your first project");t.onclick=()=>{if(!m.length)return x();q?s("#search").value="":p="All projects",S()
...
l.title="Open project & notes",l.onclick=()=>x(t),s.append(v("h3","",t.name),l);const d=v("div","card-meta"),c=v("span","")
...
e.onclick=()=>x(),e.disabled=f,n.append(e)}s("#live-region").textContent=`${l.length} projects shown.`}function M(){
s("#counter").textContent=`${b.elements.name.value.length}/60`,s("#preview-name").textContent=b.elements.name.value.trim()||"Your next lovely thing",
s("#preview-detail").textContent=`${b.elements.status.value} · ${b.elements.hoop.value.trim()||"Choose a hoop"}`}function x(e){if(!f){h=e||null,b.reset()
;for(const e of b.querySelectorAll(".error"))e.textContent="";for(const e of b.querySelectorAll("[aria-invalid]"))e.removeAttribute("aria-invalid")
;if(s("#editor-title").textContent=e?"A little work in progress.":"Start something lovely.",s("#save").textContent=e?"Save changes ↗":"Save project ↗",
e)for(const[t,n]of Object.entries(e))b.elements.namedItem(t)&&(b.elements.namedItem(t).value=n);else b.elements.started.value=d(),
b.elements.hoop.value='6" round';b.elements.started.max=d(),b.elements.finished.max=d(),M(),y.showModal(),s("#name").focus()}}function $(){try{
const e=localStorage.getItem(l);let r=null===e?c:JSON.parse(e);if(!Array.isArray(r))throw Error();const o=new Set;r=r.map(e=>{
if(!e||"object"!=typeof e||Array.isArray(e))throw Error();const r=t(e);if(!r.id||o.has(r.id)||Object.keys(n(r,d())).length)throw Error();return o.add(r.id),r}),
m=r,f=!1,s("#grid-error").textContent="",s("#retry").hidden=!0,s("#add-project").disabled=!1,s("#save").disabled=!1,S()}catch{f=!0,
s("#grid-error").textContent="We couldn’t read your saved notebook. Your original data has been kept safe. Check browser storage permissions or restore the saved data, then try reading again.",
s("#retry").hidden=!1,s("#add-project").disabled=!0,s("#save").disabled=!0,s("#grid").replaceChildren()}}s("#add-project").onclick=()=>x(),
...
const t=document.getElementById(`${e.target.name}-error`);t&&(t.textContent=""),e.target.removeAttribute("aria-invalid"),M()}),b.onsubmit=e=>{e.preventDefault()
;const r=Object.fromEntries(new FormData(b)),o=t({...r,id:h?.id||crypto.randomUUID(),art:h?.art??m.length%6}),a=n(o,d())
;for(const e of b.querySelectorAll(".error"))e.textContent="";for(const e of b.querySelectorAll("[aria-invalid]"))e.removeAttribute("aria-invalid")
;if(Object.keys(a).length){for(const[e,t]of Object.entries(a))document.getElementById(`${e}-error`).textContent=t,
b.elements[e].setAttribute("aria-invalid","true"),b.elements[e].setAttribute("aria-describedby",`${e}-error`);return void b.elements[Object.keys(a)[0]].focus()}
A(h?m.map(e=>e.id===h.id?o:e):[o,...m],"#form-error")&&(y.close(),p="All projects",s("#search").value="",S(),
C(h?"Your project is all up to date.":"A new project, a little possibility."))},s("#search").oninput=S,s("#sort").onchange=S,
document.addEventListener("pointerdown",e=>{g&&e.target!==g&&w()}),document.addEventListener("keydown",e=>{"Escape"===e.key&&w()}),s("#theme").onclick=()=>{
const e="dark"!==document.documentElement.dataset.theme;document.documentElement.dataset.theme=e?"dark":"light",s("#theme").textContent=e?"☀":"☾",
s("#theme").setAttribute("aria-label",e?"Switch to light theme":"Switch to dark theme")},s("#retry").onclick=$,s("#hero-hoop").append(i(5)),
